...
async function claimJob() {
  const res = await fetch(`${API_BASE}/crawler/next`, {
    method: "POST",
    headers: { ...authHeaders() },
  });
...
async function reportError(jobId, url, err) {
  try {
    await fetch(`${API_BASE}/crawler/${jobId}/error`, {
      method: "POST",
      headers: { ...authHeaders(), "Content-Type": "application/json" },
      body: JSON.stringify({ id: jobId, url, error: String(err) }),
    });
  } catch (e) {
...
async function handleJob(job) {
  ...
  try {
    ...
    if (!allowed) {
      ...
      await reportResult(id, []);
      return;
    }

    const res = await timeoutFetch(url, {
      method: "GET",
      headers: { "User-Agent": "kse-crawler/1.0" },
    });
    ...
    const discovered = Array.from(new Set([...links, ...sitemapLinks])).slice(
      0,
      MAX_LINKS,
    );
