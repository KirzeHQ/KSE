  ...
  try {
    for (const m of html.matchAll(
      ...
      if (urlMatch && urlMatch[1]) {
        ...
        try {
          targets.add(new URL(t, base).toString());
        } catch (e) {}
      }
    }
...
async function fetchSitemap(origin) {
  ...
  try {
    const res = await timeoutFetch(
      sitemapUrl,
      { method: "GET", headers: { "User-Agent": "kse-indexer/1.0" } },
      8000,
    );
    if (!res.ok) return [];
    const txt = await res.text();
    const locs = [...txt.matchAll(/<loc\s*>\s*([^<]+?)\s*<\/loc>/gi)].map((m) =>
      m[1].trim(),
    );
    return locs;
  } catch (e) {
...
function parseRobotsTxt(text) {
  ...
  for (const l of lines) {
    ...
    if (key === "user-agent") {
      if (cur.agents.length || cur.rules.length) {
        sections.push(cur);
        cur = { agents: [], rules: [] };
      }
...
async function fetchRobotsForOrigin(origin) {
  ...
  try {
    const res = await timeoutFetch(
      url,
      { method: "GET", headers: { "User-Agent": ROBOTS_AGENT } },
      5000,
    );
...
async function isUrlAllowed(urlStr) {
  try {
    ...
    if (!robots.sections.length)
      return { allowed: true, crawlDelay: robots.crawlDelay };
    let matchedSection = null;
    for (const s of robots.sections)
      if (s.agents.includes(ROBOTS_AGENT)) matchedSection = s;
    if (!matchedSection) {
      for (const s of robots.sections)
        if (s.agents.includes("*")) matchedSection = s;
    }
    if (!matchedSection)
      return { allowed: true, crawlDelay: robots.crawlDelay };
...
function encodeRecord(rec) {
  ...
  try {
    dateBuf.writeBigUInt64BE(BigInt(rec.crawl_date || Date.now()));
  } catch (e) {
    dateBuf.writeBigUInt64BE(BigInt(Date.now()));
  }
  parts.push(dateBuf);
...
async function flushBuffer() {
  ...
  addLog(
    `submitting batch id=${batchId} size=${batch.length} format=${SUBMIT_FORMAT} (buffer now ${discoveredLinksCount})`,
  );
  try {
    let res;
    if (SUBMIT_FORMAT === "json") {
      ...
      res = await fetch(`${API_BASE}/indexer/submit`, {
        ...
        headers: {
          ...
          "X-URL-Count": String(batch.length),
        },
        body: payload,
      });
    } else {
      ...
      res = await fetch(`${API_BASE}/indexer/submit`, {
        ...
        headers: {
          ...
          "Content-Disposition": `attachment; filename=batched_${batchId}.bin`,
          "X-URL-Count": String(batch.length),
        },
        body: bin,
      });
    ...
    if (!res || !res.ok)
      throw new Error(
        `batch submit failed: ${res ? res.status : "no-response"}`,
      );
    submissionsCount++;
    recordedCount += batch.length;
    addLog(
      `batch ${batchId} submitted OK (total submissions=${submissionsCount} total_records_sent=${recordedCount})`,
    );
  } catch (e) {
    // requeue
    ...
    failedSubmissions++;
    addLog(
      `[ERROR] batch ${batchId} submit error: ${e && e.message ? e.message : e}`,
    );
  }
}
...
function maybeFlush() {
  if (discoveredLinksCount >= BATCH_SIZE) {
    flushBuffer().catch((e) =>
      addLog(`[ERROR] flushBuffer failed: ${e && e.message ? e.message : e}`),
    );
  }
}
...
async function handleUrl(url) {
  ...
  try {
    ...
    try {
      ...
      const delayMs =
        crawlDelay && Number(crawlDelay) > 0 ? Number(crawlDelay) * 1000 : 0;
    ...
    let sitemapLinks = [];
    try {
      sitemapLinks = await fetchSitemap(new URL(url).origin);
    } catch (e) {}
    ...
    if (discovered.length)
      addLog(
        `discovered ${discovered.length} links for ${url} sample: ${JSON.stringify(discovered.slice(0, 5))}`,
      );
    ...
    addLog(
      `[ERROR] handleUrl error ${url} ${err && err.message ? err.message : err}`,
    );
    visitedCount++;
  }
}
...
async function workerLoop(workerId) {
  ...
  while (true) {
    try {
      ...
      addLog(
        `[ERROR] worker ${workerId} error ${err && err.message ? err.message : err}`,
      );
      await sleep(POLL_INTERVAL);
    }
  }
...
async function mainLoop() {
  try {
    ...
    try {
      ...
      addLog(
        `[WARN] reading seeds file error: ${e && e.message ? e.message : e}`,
      );
    }
    ...
    for (let i = 0; i < workers; i++) {
      workerLoop(i + 1).catch((e) =>
        addLog(
          `[ERROR] worker ${i + 1} crashed: ${e && e.message ? e.message : e}`,
        ),
      );
    }
    ...
    if (FLUSH_INTERVAL_MS > 0) {
      setInterval(() => {
        flushBuffer().catch((e) =>
          addLog(
            `[ERROR] periodic flush failed: ${e && e.message ? e.message : e}`,
          ),
        );
      }, FLUSH_INTERVAL_MS);
    }
    ...
    addLog(
      `mainLoop started: workers=${workers} seeds=${seeds.length} flush_interval_ms=${FLUSH_INTERVAL_MS}`,
    );
  } catch (e) {
